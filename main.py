...
if __name__ == '__main__':
    ...
    all_viruses_codons_frequencies = []
    all_viruses_dicodons_frequencies = []
    ...
    for one_file_name in all_viruses:
        for seq_record in SeqIO.parse(one_file_name, "fasta"):
            ...
            translated_dicodons = [codon + codon2
                                   for codon in translated_nucleotides
                                   for codon2 in translated_nucleotides]
            frequency_of_codons = count_frequency(all_frames_translated, translated_nucleotides)
            frequency_of_dicodons = count_frequency(all_frames_translated, translated_dicodons)
            print(f'\nthe table of codon frequencies in {seq_record.id}\n')
            print_data_in_table(frequency_of_codons.tolist(),
                                top_down_headers=translated_nucleotides,
                                left_right_headers=["codon", "Frequency"],
                                dimensions=2)
            print(f'\nthe table of dicodon frequencies in {seq_record.id}\n')
            print_data_in_table(format_numpy_array_digits(frequency_of_dicodons,
                                                          decimal_spaces=4,
            ...
            all_viruses_codons_frequencies.append((seq_record.id, frequency_of_codons))
            all_viruses_dicodons_frequencies.append((seq_record.id, frequency_of_dicodons))

    # end for - all records have been read
    # now it is time to compare
    distance_codon_in_phylip = [len(all_viruses_codons_frequencies)]
    for frequency in all_viruses_codons_frequencies:
        frequency_row = [frequency[0]]  # name of sample
        for i in range(len(all_viruses_codons_frequencies)):
            distance = calculate_difference_frequencies(frequency[1],
                                                        all_viruses_codons_frequencies[i][1])
            frequency_row.append(f'{distance:2.5f}')
        distance_codon_in_phylip.append(frequency_row)

    distance_dicodon_in_phylip = [len(all_viruses_dicodons_frequencies)]
    for frequency in all_viruses_dicodons_frequencies:
        frequency_row = [frequency[0]]  # name of sample
        for i in range(len(all_viruses_dicodons_frequencies)):
            distance = calculate_difference_frequencies(frequency[1],
                                                        all_viruses_dicodons_frequencies[i][1])
            frequency_row.append(f'{distance:2.5f}')
        distance_dicodon_in_phylip.append(frequency_row)

    print("\nCodon distance matrix in Phylip")
    print_in_phylip(distance_codon_in_phylip)

    print("\nDicodon distance matrix in Phylip")
    print_in_phylip(distance_dicodon_in_phylip)
    # task 5 done above.
